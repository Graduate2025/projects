...
class tree:
    ...
    def delete(self, Dir, name):
        ...
        if name in self.ls(Dir):
            ...
            if Dir.access[0] == 's':
                print('You can find the password in the Help pass command')
...
commands = {
    ...
    'pass':"FBI",
    'permission':"changes the permission of the file if not set by a super user, asks for a password if set by a super user (type permission, hit ENTER, write new access as rwx)",
    'sudo permission':"changes the permission of the file and adds the super user permission, requires a password (type sudo permission, enter the password, enter new acces as rwx)"
}
...
while True:
    ...
    if i=='getsize':
        root.updatesize(root.root)
        print('root block size =',root.root.block)
